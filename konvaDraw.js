...
let defaultTension = 0.5

let pencilSelect = 'pencil vector'
let eraserSelect = 'off'
...
function globalCompisteOperation() {
  return eraserSelect === 'On' ? 'destination-out' : 'source-over'
}
...
function drawLine(
  ...
  line[lineId] = new Konva.Shape({
    stroke: bezLineColor ? bezLineColor : curveColor,
    strokeWidth: 4,
    lineId: lineId,
    fill: '',
    //*************************************** */
...
function mouseOverLine(e) {
  ...
  if (currentBezShape && currentBezShape !== linesShape) {
    ...
    for (let x = 1; x < bezShapes[linesShape].bezLines.length; x++) {
      ...
      line[thisLineId].setAttr('stroke', 'orange')
    }
  }
}
...
function mouseOffLine(e) {
  ...
  for (let x = 1; x < bezShapes[linesShape].bezLines.length; x++) {
    thisLineId = linesShape + '.' + x
    line[thisLineId].setAttr('stroke', bezShapes[linesShape].bezLines[x].color)
  }
}
  ...
  for (let bezShape = 1; bezShape < bezShapes.length; bezShape++) {
    ...
    for (let x = 1; x < bezShapes[bezShape].bezLines.length; x++) {
      ...
      drawLine(
        ...
        bezShape,
        x,
        bezShapes[bezShape].bezLines[x].color
      )
